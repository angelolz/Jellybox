...
public class Help extends Command {
    ...
    protected void execute(CommandEvent event) {
        ...
        if(!event.getArgs().isEmpty()) {
            ...
            for(Command command : event.getClient().getCommands()) {
                ...
                if(commandMatches(command.getName(), aliases, event.getArgs())) {
                    embed.setTitle(String.format("Command Info for `%s%s`", Config.getPrefix(), command.getName()))
                         .addField("Arguments", command.getArguments() == null ? "*None*" : command.getArguments(),
                             true)
            ...
            EmbedBuilder embed = new EmbedBuilder().setColor(Statics.EMBED_COLOR).setTitle("Player Controls");

            getCommands(embed, "Player");

            channel.sendMessageEmbeds(embed.build())
                   .setComponents(ActionRow.of(getCategoryButtons(event.getAuthor().getId(), "player")))
                   .queue();
        }
    }
    ...
    public static void getEmbed(ButtonInteractionEvent event, String category) {
        ...
        event.deferEdit()
             ...
             .setComponents(ActionRow.of(getCategoryButtons(event.getUser().getId(), category)))
             .queue();

    }

    private static boolean commandMatches(String commandName, String[] aliases, String input) {
        return commandName.equalsIgnoreCase(input) || (aliases != null && Arrays.stream(aliases)
    ...
    private static void getCommandHelp(EmbedBuilder embed, String commandName) {
        switch(commandName) {
            case "help" ->
                embed.setDescription("Shows an embed listing all of the available commands as well as detailed help " +
                    "for each of them. " + "If you provide a command name or its alias as an argument, it would show " +
                    "the usage of the command in more detail.");
            case "join" ->
            ...
            case "queue" -> embed.setDescription("Displays all the tracks that are currently in the queue.")
                                 .addField("Subcommands", String.format("""
    ...
    private static List<Button> getCategoryButtons(String userId, String category) {
        ...
        categoryButtons.add(Button.secondary(String.format("%s:pagination:help:player", userId), Emoji.fromUnicode("U" +
                                      "+1F3B5"))
                                  .withLabel("Player")
                                  .withDisabled(category.equalsIgnoreCase("player")));
        categoryButtons.add(Button.secondary(String.format("%s:pagination:help:bot", userId), Emoji.fromUnicode("U" +
                                      "+1F916"))
                                  .withLabel("Bot")
                                  .withDisabled(category.equalsIgnoreCase("bot")));
